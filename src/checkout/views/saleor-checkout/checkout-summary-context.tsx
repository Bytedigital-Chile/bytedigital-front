...
export function buildShippingSummaryRows(checkout: CheckoutFragment): SummaryRow[] {
	return [
		{ label: "Contacto", value: checkout.email || "", onChangeStep: 1 },
		{ label: "Enviar a", value: formatAddressLine(checkout.shippingAddress), onChangeStep: 1 },
	];
}

/** Build standard summary rows for payment step */
export function buildPaymentSummaryRows(checkout: CheckoutFragment): SummaryRow[] {
	const rows: SummaryRow[] = [{ label: "Contacto", value: checkout.email || "", onChangeStep: 1 }];

	// Only show shipping info for physical products
	if (checkout.isShippingRequired) {
		rows.push(
			{ label: "Enviar a", value: formatAddressLine(checkout.shippingAddress), onChangeStep: 1 },
			{ label: "Método", value: formatShippingMethod(checkout), onChangeStep: 2 },
		);
	} else {
		// Digital products - show delivery type instead
		rows.push({ label: "Entrega", value: "Digital" });
	}
